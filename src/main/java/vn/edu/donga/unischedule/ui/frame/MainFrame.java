...
public class MainFrame extends JFrame implements ScreenNavigator {
    ...
    public MainFrame(AppServices services, User currentUser) {
    ...
    private JPanel buildSidebar() {
        ...
        create.addActionListener(e -> {
            if (currentUser.getRole() == Role.ACADEMIC) {
                vn.edu.donga.unischedule.ui.dialog.ScheduleFormDialog.showDialog(this, services, null).ifPresent(entry -> {
                    try { services.schedules().save(entry); showScreen("timetable"); }
    ...
    private void registerScreens() {
        addScreen("dashboard", "Tổng quan điều hành", new DashboardPanel(services, currentUser, this));
        timetableScreen = new TimetablePanel(services, currentUser);
        addScreen("timetable", "Thời khóa biểu", timetableScreen);
        addScreen("conflicts", "Xung đột thời khóa biểu", new ConflictPanel(services));
        addScreen("courseSections", "Lớp học phần", new CourseSectionPanel(services, currentUser));
        addScreen("rooms", "Phòng và thiết bị", new RoomManagementPanel(services, currentUser));
        addScreen("requests", "Quản lý yêu cầu", new RequestManagementPanel(services, currentUser));
        addScreen("roomSearch", "Tra cứu phòng trống", new RoomSearchPanel(services, currentUser));
        addScreen("users", "Quản lý người dùng", new UserManagementPanel(services));
        addScreen("audit", "Nhật ký hoạt động", new AuditLogPanel());
        addScreen("notifications", "Thông báo", new NotificationPanel(services, currentUser, this));
        addScreen("profile", "Hồ sơ và cài đặt", new ProfilePanel(services, currentUser));
    }
    ...
    private void refreshNotificationButton() {
        long unread = services.notifications().unreadCount(currentUser);
