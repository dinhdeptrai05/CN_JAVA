...
public class RequestFormDialog extends JDialog {
    ...
    public RequestFormDialog(JFrame owner, AppServices services, User user) {
        super(owner, "Gửi yêu cầu", true);
        this.user = user;
        services.schedules().findByWeekForUser(DateUtils.currentWeekMonday(), user).forEach(scheduleBox::addItem);
        services.rooms().findAll().forEach(roomBox::addItem);
        services.catalog().getTimeSlots().forEach(slotBox::addItem);
        buildUi();
        setSize(620, 560);
    ...
    public static Optional<ChangeRequest> showDialog(Component parent, AppServices services, User user) {
        JFrame owner = (JFrame) javax.swing.SwingUtilities.getWindowAncestor(parent);
        RequestFormDialog dialog = new RequestFormDialog(owner, services, user);
    ...
    private void save() {
        try {
            int quantity = quantityField.getText().isBlank() ? 0 : Integer.parseInt(quantityField.getText().trim());
            result = new ChangeRequest(null, user, (RequestType) typeBox.getSelectedItem(),
                    (ScheduleEntry) scheduleBox.getSelectedItem(), (Classroom) roomBox.getSelectedItem(),
                    LocalDate.parse(dateField.getText().trim(), DateUtils.INPUT_FORMAT),
                    (TimeSlot) slotBox.getSelectedItem(), equipmentField.getText().trim(), quantity,
                    reasonArea.getText().trim(), (Priority) priorityBox.getSelectedItem(),
