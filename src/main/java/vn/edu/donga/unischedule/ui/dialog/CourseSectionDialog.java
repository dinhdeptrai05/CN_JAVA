...
public class CourseSectionDialog extends JDialog {
    ...
    public CourseSectionDialog(JFrame owner, AppServices services, CourseSection editing) {
        super(owner, editing == null ? "Thêm lớp học phần" : "Sửa lớp học phần", true);
        this.editing = editing;
        services.catalog().getCourses().forEach(courseBox::addItem);
    ...
    public static Optional<CourseSection> showDialog(Component parent, AppServices services, CourseSection editing) {
        JFrame owner = (JFrame) javax.swing.SwingUtilities.getWindowAncestor(parent);
        CourseSectionDialog dialog = new CourseSectionDialog(owner, services, editing);
    ...
    private void save() {
        try {
            result = new CourseSection(editing == null ? null : editing.getId(), codeField.getText().trim(),
                    ...
                    Integer.parseInt(studentCountField.getText().trim()),
