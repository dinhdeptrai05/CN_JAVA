...
public class UserFormDialog extends JDialog {
    ...
    public UserFormDialog(JFrame owner, AppServices services, User editing) {
    ...
    public static Optional<User> showDialog(Component parent, AppServices services, User editing) {
        JFrame owner = (JFrame) javax.swing.SwingUtilities.getWindowAncestor(parent);
        UserFormDialog dialog = new UserFormDialog(owner, services, editing);
    ...
    private void save() {
        ...
        User user = services.users().createUser(role, usernameField.getText().trim(),
                fullNameField.getText().trim(), emailField.getText().trim(), phoneField.getText().trim());
        if (editing != null) {
            user.setId(editing.getId());
            user.setPassword(editing.getPassword());
        }
        user.setStatus((UserStatus) statusBox.getSelectedItem());
        result = user;
        dispose();
    }
}
