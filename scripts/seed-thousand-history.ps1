...
try {
    ...
    & mvn -q compile exec:java '-Dexec.mainClass=vn.edu.donga.unischedule.simulation.HistoricalThousandSeed' "-Dexec.args=$mode"
    if($LASTEXITCODE -ne 0){throw 'Five-year 1000-row seed failed.'}
} finally {
