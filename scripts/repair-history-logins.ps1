...
try {
    ...
    & mvn -q compile exec:java '-Dexec.mainClass=vn.edu.donga.unischedule.simulation.HistoricalLoginTimelineRepair'
    if($LASTEXITCODE -ne 0){throw 'Historical login timeline repair failed.'}
} finally {
