...
try {
    ...
    & mvn -q compile exec:java '-Dexec.mainClass=vn.edu.donga.unischedule.simulation.HistoricalSemesterRenamer'
    if($LASTEXITCODE -ne 0){throw 'Historical semester rename failed.'}
} finally {
