...
public final class JdbcNotificationRepository implements NotificationRepository {
    ...
    public Notification save(Notification n) {
        return db.transaction(c->{
            ...
            int changed=db.update("UPDATE notifications SET is_read=?,read_at=CASE WHEN ? THEN UTC_TIMESTAMP() ELSE NULL END WHERE id=? AND user_id=? AND is_read<>?",n.isRead(),n.isRead(),n.getId(),db.actor().getId(),n.isRead());
            if(changed>0) db.audit(n.isRead()?"READ_NOTIFICATION":"UNREAD_NOTIFICATION","notifications",n.getId());
            return n;
        });
    }
    public boolean deleteById(Long id) {
        return db.transaction(c->{
            db.require(Role.values());
            int changed=db.update("DELETE FROM notifications WHERE id=? AND user_id=?",id,db.actor().getId());
            if(changed>0) db.audit("DELETE_NOTIFICATION","notifications",id);
            return changed>0;
        });
    }
}
