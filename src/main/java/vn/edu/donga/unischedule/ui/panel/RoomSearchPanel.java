...
public class RoomSearchPanel extends JPanel implements Refreshable {
    ...
    public RoomSearchPanel(AppServices services, User user) {
    ...
    public void refresh() {
        try {
            ...
            latestRows = services.rooms().searchAvailableRooms(date, (TimeSlot) slotBox.getSelectedItem(),
    ...
    private String equipmentSummary(Classroom room) {
        String text = services.rooms().findEquipmentByRoom(room.getId()).stream()
    ...
    private void createRequest(Classroom room) {
        try {
            ChangeRequest request = new ChangeRequest(null, user, RequestType.USE_ROOM, null, room,
                    LocalDate.parse(dateField.getText().trim(), DateUtils.INPUT_FORMAT),
                    (TimeSlot) slotBox.getSelectedItem(), "", 0,
