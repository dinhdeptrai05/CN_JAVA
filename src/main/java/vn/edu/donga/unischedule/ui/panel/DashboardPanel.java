...
public class DashboardPanel extends JPanel implements Refreshable {
    ...
    public DashboardPanel(AppServices services, User user) {
        this(services, user, key -> { });
    }

    public DashboardPanel(AppServices services, User user, ScreenNavigator navigator) {
    ...
    public void refresh() {
        ...
        services.catalog().getDepartments().forEach(d -> departments.add(d.getName()));
    ...
    private void buildStats(JPanel stats) {
        ...
        if (user.getRole() == Role.ADMIN) {
            stats.add(new StatCard("ND", "Tài khoản", String.valueOf(services.users().findAll().size()), "Người dùng demo", AppConfig.PRIMARY));
            stats.add(new StatCard("TR", "Phòng trống", String.valueOf(available), "Sẵn sàng sử dụng", AppConfig.SUCCESS));
            stats.add(new StatCard("SD", "Đang dùng", String.valueOf(inUse), "Có lịch trong tuần", AppConfig.WARNING));
            stats.add(new StatCard("BT", "Bảo trì", String.valueOf(maintenance), "Cần theo dõi", AppConfig.DANGER));
            ...
            stats.add(new StatCard("LH", "Tổng số lớp tuần này", String.valueOf(weekClasses), "Lớp có lịch trong tuần", AppConfig.PRIMARY));
            stats.add(new StatCard("PH", "Tỷ lệ sử dụng phòng", (totalRooms == 0 ? 0 : inUse * 100 / totalRooms) + "%", inUse + "/" + totalRooms + " phòng đang dùng", Color.decode("#0EA5E9")));
            stats.add(new StatCard("YC", "Đổi lịch chờ duyệt", String.valueOf(pending), "Yêu cầu đang chờ", AppConfig.WARNING));
            stats.add(new StatCard("XD", "Cảnh báo xung đột", String.valueOf(services.conflicts().findAllConflicts().stream().filter(c -> c.getStatus() != vn.edu.donga.unischedule.model.Enums.ConflictStatus.RESOLVED).count()), "Cần xử lý", AppConfig.DANGER));
        } else {
            ...
            String next = week.stream().findFirst()
                    ...
                    .orElse("Chưa có lịch");
            stats.add(new StatCard("HN", "Hôm nay", String.valueOf(todayCount), "Buổi học/dạy", AppConfig.PRIMARY));
            stats.add(new StatCard("TT", "Buổi tiếp theo", todayCount > 0 ? "Có lịch" : "Trống", next, AppConfig.SUCCESS));
            stats.add(new StatCard("TB", "Thông báo mới", String.valueOf(services.notifications().unreadCount(user)), "Cần đọc", AppConfig.WARNING));
            stats.add(new StatCard("TU", "Trong tuần", String.valueOf(week.size()), "Lịch liên quan", AppConfig.PRIMARY));
        }
    }
    ...
    private JTable buildTodayTable() {
        ...
        int today = DateUtils.toSchoolDay(LocalDate.now());
        model.setRows(services.schedules().findByWeekForUser(DateUtils.currentWeekMonday(), user).stream()
    ...
    private JTable buildRequestTable() {
        GenericTableModel<ChangeRequest> model = new GenericTableModel<>(
                new String[]{"Mã", "Người gửi", "Loại", "Ưu tiên", "Trạng thái"},
                ...
                request -> request.getStatus().getDisplayName());
        model.setRows(services.requests().findForUser(user).stream().limit(8).toList());
        JTable table = new JTable(model);
        TableUtils.style(table);
        return table;
    }
    ...
    private JPanel buildUsagePanel() {
        ...
        services.rooms().findAll().stream().map(room -> room.getBuilding()).distinct().forEach(building -> {
            var rooms = services.rooms().findAll().stream().filter(room -> room.getBuilding().equals(building)).toList();
            long used = rooms.stream().filter(room -> room.getRoomStatus() == RoomStatus.IN_USE).count();
            int percent = rooms.isEmpty() ? 0 : (int) (used * 100 / rooms.size());
            JPanel row = new JPanel(new BorderLayout(0, 6)); row.setOpaque(false);
            row.add(new JLabel(building + "  ·  " + used + "/" + rooms.size() + " phòng"), BorderLayout.NORTH);
            JProgressBar progress = new JProgressBar(0, 100); progress.setValue(percent);
    ...
    private JPanel buildConflicts() {
        ...
        services.conflicts().findAllConflicts().stream()
                .filter(c -> c.getStatus() != vn.edu.donga.unischedule.model.Enums.ConflictStatus.RESOLVED)
                .limit(2).forEach(conflict -> {
    ...
    private JPanel buildRequests() {
        ...
        services.requests().findForUser(user).stream().sorted(java.util.Comparator.comparing(ChangeRequest::getCreatedAt).reversed()).limit(3).forEach(request -> {
            ...
            if (services.requests().canProcess(request, user)) {
                javax.swing.JButton approve = new vn.edu.donga.unischedule.ui.component.PrimaryButton("Duyệt yêu cầu");
                approve.addActionListener(e -> { services.requests().approve(request, user); refresh(); });
    ...
    private JPanel buildNotificationPanel() {
        ...
        panel.setOpaque(false);
        services.notifications().findForUser(user).stream().limit(5).forEach(notification -> {
