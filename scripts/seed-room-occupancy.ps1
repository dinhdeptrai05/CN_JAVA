...
try {
    ...
    & mvn -q compile exec:java '-Dexec.mainClass=vn.edu.donga.unischedule.simulation.RoomOccupancySeed' "-Dexec.args=$mode"
    if($LASTEXITCODE -ne 0){throw 'Room occupancy seed failed.'}
} finally {
