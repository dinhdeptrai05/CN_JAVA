...
public class RoomDetailDialog extends JDialog {
    ...
    public RoomDetailDialog(JFrame owner, AppServices services, Classroom room, User user) {
    ...
    public static void showDialog(Component parent, AppServices services, Classroom room, User user) {
        JFrame owner = (JFrame) javax.swing.SwingUtilities.getWindowAncestor(parent);
        new RoomDetailDialog(owner, services, room, user).setVisible(true);
    }
    ...
    private JTable buildScheduleTable() {
        ...
        model.setRows(services.schedules().findAll().stream()
    ...
    private void refreshEquipment() {
        equipmentModel.setRows(services.rooms().findEquipmentByRoom(room.getId()));
    }
    ...
    private void addEquipment() {
        EquipmentDialog.showDialog(this, room, null).ifPresent(item -> {
            try {
                services.rooms().saveEquipment(item);
                refreshEquipment();
    ...
    private void updateQuantity() {
        try {
            ...
            item.setQuantity(Integer.parseInt(input.trim()));
            services.rooms().saveEquipment(item);
            refreshEquipment();
    ...
    private void markBroken() {
        try {
            Equipment item = selectedEquipment();
            item.setStatus(ResourceStatus.BROKEN);
            item.setCondition("Đang chờ xử lý");
            services.rooms().saveEquipment(item);
            refreshEquipment();
