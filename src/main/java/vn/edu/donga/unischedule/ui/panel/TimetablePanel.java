...
public class TimetablePanel extends JPanel implements Refreshable {
    ...
    public TimetablePanel(AppServices services, User user) {
    ...
    private void buildFilters() {
        ...
        for (Semester semester : services.catalog().getSemesters()) {
            semesterBox.addItem(semester.getName());
        }
        departmentBox.addItem("Tất cả khoa");
        for (Department department : services.catalog().getDepartments()) {
            departmentBox.addItem(department.getName());
        }
        lecturerBox.addItem("Tất cả giảng viên");
        services.catalog().getLecturers().forEach(lecturer -> lecturerBox.addItem(lecturer.getFullName()));
        roomBox.addItem("Tất cả phòng");
        services.rooms().findAll().forEach(room -> roomBox.addItem(room.getCode()));
    ...
    private List<ScheduleEntry> filteredRows() {
        ...
        return services.schedules().findByWeekForUser(weekStart, user).stream()
    ...
    private void rebuildCalendar(List<ScheduleEntry> rows) {
        ...
        JPanel grid = new JPanel(new GridLayout(services.catalog().getTimeSlots().size() + 1, 8, 1, 1));
    ...
    private void addSchedule() {
        ScheduleFormDialog.showDialog(this, services, null).ifPresent(entry -> {
            try {
                services.schedules().save(entry);
                refresh();
    ...
    private void editSchedule() {
        try {
            ...
            ScheduleFormDialog.showDialog(this, services, entry).ifPresent(updated -> {
                try {
                    services.schedules().save(updated);
                    refresh();
    ...
    private void deleteSchedule() {
        try {
            ...
            if (Dialogs.confirm(this, "Xóa lịch " + entry.getCourseSection().getCode() + " khỏi dữ liệu giả?")) {
                services.schedules().delete(entry.getId());
                refresh();
