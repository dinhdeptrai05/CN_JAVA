    ...
    @Test void renderScreensAndForms() throws Exception {
        SwingUtilities.invokeAndWait(() -> {
            ...
            try {
                ...
                JDialog[] forms = {new ScheduleFormDialog(frame, new vn.edu.donga.unischedule.controller.AppControllers(services), null), new RoomFormDialog(frame, null),
                        new EquipmentDialog(frame, services.rooms().findAll().get(0), null),
                        new CourseSectionDialog(frame, new vn.edu.donga.unischedule.controller.AppControllers(services), null), new UserFormDialog(frame, new vn.edu.donga.unischedule.controller.AppControllers(services), null),
                        new RequestFormDialog(frame, new vn.edu.donga.unischedule.controller.AppControllers(services), services.auth().login("giangvien", "123456")),
                        new RoomDetailDialog(frame, new vn.edu.donga.unischedule.controller.AppControllers(services), services.rooms().findAll().get(0), services.auth().login("admin", "123456"))};
                for (JDialog form : forms) {
                    try { form.addNotify(); render(form, form.getClass().getSimpleName()); }
                    finally { form.dispose(); }
                }
                LoginFrame login = new LoginFrame(new vn.edu.donga.unischedule.controller.AppControllers(services));
                try { login.addNotify(); render(login, "login"); } finally { login.dispose(); }
                for (String account : new String[]{"admin", "giangvien", "sinhvien"}) {
                    MainFrame roleFrame = new MainFrame(new vn.edu.donga.unischedule.controller.AppControllers(services), services.auth().login(account, "123456"));
                    try {
